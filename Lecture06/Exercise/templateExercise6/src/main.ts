import { scoreQuestion, calculateFinalScore, resetScores } from "./modules/scoringModule.js";
import { Question, QuestionModule } from "./modules/questionModule.js";
import { switchToInput, switchToQuiz, setCurrentQuestion, switchToScore, generateScore, renderLeaderboard } from "./modules/uiModule.js";
...
(document.getElementById("new-game") as HTMLButtonElement).addEventListener("click", () => {
    player = "";
    currentQuestion = 0;
...
async function buttonNext() {
    ...
    if (currentQuestion > (questions.length - 1)) {
        ...
        await leaderboard.addScore(player, finalScore!.points, (finalScore!.points / finalScore!.totalPoints));
...
(document.getElementById("option-1") as HTMLButtonElement).addEventListener("click", () => {
    scoreQuestion(questions[currentQuestion], player, 0);
    buttonNext();
});

(document.getElementById("option-2") as HTMLButtonElement).addEventListener("click", () => {
    scoreQuestion(questions[currentQuestion], player, 1);
    buttonNext();
});

(document.getElementById("option-3") as HTMLButtonElement).addEventListener("click", () => {
    scoreQuestion(questions[currentQuestion], player, 2);
    buttonNext();
});

(document.getElementById("option-4") as HTMLButtonElement).addEventListener("click", () => {
